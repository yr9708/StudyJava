...
//		prn(school);
	...
	public static void trans(Set<Score> set) {
		// set에서 이름이 홍길동인 Score객체를 찾아서 국어점수를 0점으로 변경 후 전체 출력하자.
		// 단, iterator를 사용하자.
		
		Iterator<Score> it = set.iterator();
		while(it.hasNext()) {
			Score tmp = it.next();
	...
	public static void prn(Set<Score> set) {
		...
		for(int i =0;i<arr.length; i++) {
			System.out.println(((Score)arr[i]).getName());		// 작은(Score)type-> 큰(Object) type 으로 변환해줄때 명시적으로 변환
		}
		System.out.println("----------");
		
		// 방법 3 : collection 내부의 값을 순회하는 표준화된 방법
